...
def Database():
    ...
    conn = sqlite3.connect("ข้อมูลสินค้า.db")
    cursor = conn.cursor()
...
def DisplayForm():
    ...
    Label(LFrom, text="ชื่อ  ", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom,font=("Arial",10,"bold"),textvariable=name).pack(side=TOP, padx=10, fill=X)
    Label(LFrom, text="เบอร์โทรศัพท์ ", font=("Arial", 12)).pack(side=TOP)
   ...
   #setting scrollbar
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "ชื่อ", "Contact", "Email","Rollno","Branch"),
    ...
    tree.heading('ชื่อ', text="Name", anchor=W)
